    ...
    const uint8_t * const key, const uint8_t * const hash)
{
  ...
  uint8_t bitmap[DB_CONTAINER_NR / 8] __attribute__ ((aligned(8))) = {0};
  if (vc->cc.bc) {
...
db_stat_show(struct DB * const db, FILE * const fo)
{
  ...
  void
db_stat_clean_bc(struct DB * const db) {
  db->stat.nr_read_bc_count = 0;
  db->stat.nr_read_count = 0;
}
