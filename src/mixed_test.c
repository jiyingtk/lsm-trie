...
struct DBParams {
  ...
  uint64_t nr_report;
  uint64_t write_size;  //B
};

static const uint64_t nr_configs = 1;
static struct DBParams pstable[] = {
  //tag    vlen  meta_dir       cm_conf_fn     th  pw   gen        range                     sec   nr          write_size
  {"Dummy", 100, "/root/hgfs/Download/Source/lsm-trie-release-master/tmp", "cm_conf_vm.txt", 1, 100, "uniform", UINT64_C(0x100000000000), 60, 4000, 50 * 1024 * 1024},
};
...
show_dbparams(const struct DBParams * const ps)
{
  ...
  printf("    -s #write_size[MB]:  %" PRIu64 "\n", ps->write_size / 1024 / 1024);

  fflush(stdout);
}

struct DB * test_for_pause(struct DB * const db) {
  return db;
}
...
mixed_worker(const struct DBParams * const ps)
{
  ...
  uint64_t counter = 0;
  uint64_t max_insert_num = 1009200;
  ...
  // loop
  while (__ts.test_running && cur_write_size < ps->write_size) {
  // while (__ts.test_running) {
    // random keys
#ifndef LOOKUP_CORRECT_TEST
    for (uint64_t i = 0; i < 100u; i++) {
    ...
    if (ps->p_writer > 0) {
      ...
      // assert(r);
      for (int ii = 0; ii < ps->p_writer; ii++) {
        db_insert(__ts.db, kvs + ii);
        cur_write_size += (kvs + ii)->klen + (kvs + ii)->vlen;
      }
    }
#else
    //sequential keys for lookup test
    for (uint64_t i = 0; i < 100u; i++) {
      if (i < ps->p_writer) {
        keys[i] = counter++;
        uint64_t *v0 = (uint64_t *) __ts.buf;
        *v0 = keys[i];

// fprintf(stderr, "insert key %lu\n", keys[i]);fflush(stderr);
        db_insert(__ts.db, kvs + i);
// fprintf(stderr, "insert key %lu ok\n", keys[i]);fflush(stderr);
        cur_write_size += (kvs + i)->klen + (kvs + i)->vlen;
      }
      else
        keys[i] = random_uint64() % counter;
        // keys[i] = counter++;
    }
  #endif

// if (counter >= max_insert_num)
//   break;
...
// fprintf(stderr, "lookup key %lu ok\n", keys[i]);fflush(stderr);
      ...
      if (kv) {
        ...
        fprintf(stderr, "lookup key %lu not found, i=%lu !!!\n", keys[i], i);
        test_for_pause(__ts.db);
        struct KeyValue * const kv2 = db_lookup(__ts.db, sizeof(keys[i]), (const uint8_t *)(&(keys[i])));
        fflush(stderr);
        assert(false);
        exit(1);
      }
...
mixed_test(const struct DBParams * const p)
{
  ...
  for (uint64_t i = 0; i < nth; i++) {
    ...
    assert(rc == 0);
    char th_name[128] = {0};
    sprintf(th_name, "worker[%"PRIu64"]", i);
    pthread_setname_np(pth[i], th_name);
  }
...
main(int argc, char ** argv)
{
  ...
  while ((opt = getopt(argc, argv,
    ...
    switch(opt) {
      ...
      case 's': ps.write_size  = strtoull(optarg, NULL, 10) * 1024 * 1024; break;
